...
object HabitRepositoryModule {
    ...
    @Singleton
    fun repository(
        habitsDao: HabitDao, entriesDao: EntriesDao, mapper: HabitDBToDomainMapper
    ): HabitRepository =
        BaseHabitRepository(habitDao = habitsDao, entryDao = entriesDao, habitMapper = mapper)
    ...
    @Provides
    @Singleton
    fun habitDomainMapper(now: Now): HabitDBToDomainMapper = HabitDBToDomainMapper.Base(now)
